...
export const CellMember = (props: ICellMember) => {
  ...
  const renderMember = (member: IMember) => (
    <CellMemberStyled className={cellClassName} style={cellStyle} key={member.id}>
      {member.avatar ? (
        <AvatarStyled avatar={member.avatar} />
      ) : (
        <AvatarStyled bg={getAvatarRandomColor(member.id)}>{getFirstWordFromString(member.name.trim()).toUpperCase()}</AvatarStyled>
      )}
      <NameStyled>{member.name}</NameStyled>
    </CellMemberStyled>
  );
  return (
