...
export const Avatar: FC<React.PropsWithChildren<IAvatarProps>> = ({ size = 'm', icon, src, alt, children, style, shape = 'circle', onClick }) => {
  const avatarNodeRef = useRef<HTMLSpanElement>(null);
  const avatarChildrenRef = useRef<HTMLElement>(null);
  const [scale, setScale] = useState(1);
  let childrenToRender;
  if (typeof src === 'string') {
    childrenToRender = <Box as="img" width="100%" height="100%" src={src} alt={alt} display="flex" />;
  } else if (icon) {
