...
export const TitleField = (
  props: Pick<FieldProps, 'id' | 'title' | 'required'> & {
    help?: IHelp;
    hasCollapse?: boolean;
  ...
  return (
    <TitleComponent id={id} hasCollapse={hasCollapse} onClick={switchCollapse}>
      {title}
      {help && <HelpIconButton help={help} />}
      {hasCollapse && (
        <SuffixIcon isIconRotate={!collapse}>
          {' '}
          <DropIcon color="#8C8C8C" />
        </SuffixIcon>
      )}
    </TitleComponent>
  );
