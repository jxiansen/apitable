...
export const List = React.forwardRef(({ children, header, footer, data, renderItem, ...resetProps }: IListProps, ref: React.Ref<HTMLDivElement>) => {
  const renderInnerItem = (d: string | IListItemProps, index: number) => {
    return renderItem ? renderItem(d, index) : typeof d === 'string' ? <ListItem key={index}>{d}</ListItem> : <ListItem key={index} {...d} />;
  };
  return (
